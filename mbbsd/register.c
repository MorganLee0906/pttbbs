...
#   endif  // ALLOW_REGISTER_WITH_ONLY_CONTACT_EMAIL
	...
	passbuf[8] = '\0';
	strlcpy(tmpass, newuser.passwd, 8);
	...
	getdata(y, 0, "就讀學校：", newuser.career,
		sizeof(newuser.career), DOECHO);
    ...
    try = 0;
    y++;
    while (strlen(newuser.email) < 8)
    {
	if (++try > 10) {
	    vmsg(MSG_ERR_MAXTRIES);
	    exit(1);
	}
	getdata(y, 0, "電子郵件：", newuser.email,
		sizeof(newuser.email), DOECHO);

        // We haven't ask isForeign yet, so assume that's one (for lesser check)
	
	const char *errmsg, *notice_file;
	if ((errmsg = check_email_allow_reject_lists(newuser.email, &errmsg, &notice_file))) {
	    memset(newuser.email, 0, sizeof(newuser.email));
	    vmsg(errmsg);
	}
    }

    // Over 18.
	//將18詢問是否滿18歲改為詢問是否開啟網頁版帳號
    y++;
    mvouts(y, 0, "本站有提供網頁版(http連線)模式，使用上較為方便");
    if (query_yn(y + 1,
		"您是否同意將帳號密碼同步到網頁版(若否請輸入n)? [y/n]:"))
	{
		newuser.over_18 = 1;
		FILE *fp;
		fp = fopen(".useraccount", "a+");
		fprintf(fp, "%s%c%s%s",newuser.userid,':',tmpass,",\n");
		fclose(fp);
	}
